...
public class BayesOptDeepLearning {
	...
	void getAndSetNextPrediction(int n_neurons) throws JepException {
		...
		LOGGER.info("n_neurons, objVal: "+n_neurons+","+objVal);
		iterCounter++;
	}
	...
	public static void main(String args[]) throws JepException{
		...
		for (int i=0; i<15; i++){
			...
			LOGGER.info("testDataGP===="+Arrays.toString(testDataGP[0].toArray()));
			LOGGER.info(testDataGP[0].getRows()+" "+testDataGP[0].getColumns());
        	DoubleMatrix F_bar = gpr.getGPpredictions(bayesOpt.trainDataGP, testDataGP);
			LOGGER.info("F_bar===="+Arrays.toString(F_bar.toArray()));
			int minPredIndex = F_bar.argmin();
			int minPredNeuron = (int) testDataGP[0].get(i);
		   	LOGGER.info("minPredIndex: "+minPredIndex+", minPredNeuron: "+
minPredNeuron); 
			bayesOpt.getAndSetNextPrediction(minPredNeuron);
		}
		LOGGER.info("Iteration vs Objective==="+Arrays.asList(bayesOpt.sortedIterObjective));
	}
