...
from Grid  import *
import ExpectedImprovementGeneric as module
from pb2   import *
#import branin_objective as job_module
import mnist_keras as job_module
...
variable.append(ExpVar("prob_drop_out", 1, Experiment.ParameterSpec.FLOAT, 0, 1))
variable.append(ExpVar("n_hidden_layers", 1, Experiment.ParameterSpec.INT, 1, 10))
variable.append(ExpVar("n_neurons", 1, Experiment.ParameterSpec.INT, 1, 100))
...
def run_spearmint_experiment():
	put_prior = False
	if len(sys.argv)>1:
		put_prior = (sys.argv[1]=='--putprior')
	chooser.setPrior(put_prior) 
...
def runJob():
	...
	chooser.set_expt_grid(expt_grid)
	...
	print "job id: ", job_id #, "params: ", expt_grid.get_params(job_id)
	for params in expt_grid.get_params(job_id):
		print(str(params) + '\n')
	...
	for param in job.param:
		if len(param.int_val) >0 : # is not None:
			params[param.name] = np.array(param.int_val)
			#print "int_val params: ", param.int_val

		if len(param.dbl_val) > 0 : # is not None:
